...
Chunking uses token-based measurement (`length_function=count_tokens`) rather than
character length. Token counting queries Anthropic's `count_tokens` API when configured,
with a local subword tokenizer fallback for offline environments and CI testing.
Defaults are 250 tokens with 40 tokens of overlap to match the 256-token sequence limit
of `all-MiniLM-L6-v2`.
...
4. **Local Execution & Low Latency**: Runs in-process on CPU (~20-40ms latency) without dispatching secondary LLM calls.
